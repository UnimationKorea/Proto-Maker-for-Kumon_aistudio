...
const ActivityCanvas: React.FC<ActivityCanvasProps> = ({ stage, onNext, onPrev, isFirst, isLast, stageInfo }) => {
  ...
  // 1. 오토 스케일링 엔진 - 상하단 UI 공간을 충분히 확보하도록 패딩 조정
  useEffect(() => {
    const handleResize = () => {
      if (!containerRef.current?.parentElement) return;
      const parent = containerRef.current.parentElement;
      
      // 상단(내비) + 하단(컨트롤러) 공간을 위해 Y 패딩을 키움
      const paddingX = 80;
      const paddingY = 200; 
      
      const s = Math.min(
        (parent.clientWidth - paddingX) / MASTER_WIDTH,
        (parent.clientHeight - paddingY) / MASTER_HEIGHT
      );
      setAutoScale(s > 0 ? s : 0.65);
    };
  ...
  const getVerticalCenterOffset = () => -40;
  ...
  const renderDragStage = () => {
    ...
    const cardH = 380 * cs;
    fCanvas.add(new fabric.Rect({
      left: centerX, top: centerY + 50, width: cardW, height: cardH,
    ...
    if (stage.tokens) {
      ...
      stage.tokens.forEach((token, i) => {
        ...
        fCanvas.add(new fabric.Text(token.char, {
          left: x, top: y + (30 * cs), fontSize: hanjaSize * cs, fontWeight: 700, fill: '#1e293b', originX: 'center', selectable: false
        }));

        if (token.fixed) {
          fCanvas.add(new fabric.Text(token.pinyin, {
            left: x, top: y - (30 * cs), fontSize: pinyinSize * cs, fontWeight: 700, fill: '#4f46e5', originX: 'center', originY: 'center', selectable: false
          }));
        } else {
          const slot = new fabric.Rect({
            left: x, top: y - (30 * cs), width: 130 * cs, height: 70 * cs,
          ...
          fCanvas.add(slot);
          stateRef.current.dropTargets.push({ x, y: y - (30 * cs), slot });
        }
      });
    }

    fCanvas.add(new fabric.Rect({
      left: centerX, top: 120, width: 1000 * cs, height: 90 * cs, fill: '#f1f5f9', rx: 20 * cs, ry: 20 * cs, originX: 'center', originY: 'center', selectable: false
    }));
    ...
    if (stage.sourceItems) {
      ...
      stage.sourceItems.forEach((item, i) => {
        const group = new fabric.Group([
          new fabric.Rect({ width: 130 * cs, height: 60 * cs, fill: '#ffffff', stroke: '#e2e8f0', rx: 12 * cs, ry: 12 * cs, originX: 'center', originY: 'center', shadow: '0 4px 10px rgba(0,0,0,0.08)' }),
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center overflow-hidden py-4">
      {/* 1. 스테이지 및 좌우 화살표: 맨 위 상단으로 위치이동 */}
      <div className="absolute top-4 left-0 right-0 flex justify-center items-center gap-6 z-[60]">
        <button onClick={onPrev} disabled={isFirst} className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-xl border border-slate-200 disabled:opacity-20 hover:bg-indigo-600 hover:text-white transition-all group active:scale-95">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="group-hover:translate-x-[-2px] transition-transform"><polyline points="15 18 9 12 15 6" /></svg>
        </button>
        <div className="bg-white px-8 py-3 rounded-2xl shadow-xl border border-indigo-100 min-w-[140px] text-center flex flex-col justify-center">
          <span className="text-indigo-600 font-black tracking-widest text-[13px] uppercase">STAGE {stageInfo}</span>
        </div>
        <button onClick={onNext} disabled={isLast} className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-xl border border-slate-200 disabled:opacity-20 hover:bg-indigo-600 hover:text-white transition-all group active:scale-95">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="group-hover:translate-x-[2px] transition-transform"><polyline points="9 18 15 12 9 6" /></svg>
        </button>
      </div>

      {/* 2. 화면 스케일 조절 버튼 (고정, 우측 상단 - 내비게이션 하단) */}
      <div className="absolute top-20 right-10 z-[60] flex flex-col gap-2">
        <div className="bg-white/95 backdrop-blur border border-slate-200 p-2.5 rounded-2xl shadow-xl flex flex-col items-center gap-2">
          <button onClick={() => setUserZoom(p => Math.min(p + 0.05, 1.5))} className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-all font-black text-lg">+</button>
          <div className="text-[11px] font-black text-indigo-600 py-1">{Math.round(userZoom * 100)}%</div>
          <button onClick={() => setUserZoom(p => Math.max(p - 0.05, 0.4))} className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-all font-black text-lg">-</button>
        </div>
      </div>

      {/* 3. 학습 영역: 상하 센터 및 상단으로 위치 조정 */}
      <div className="flex-1 flex items-center justify-center w-full mt-10">
        <div 
          ref={containerRef}
          className="relative bg-white shadow-2xl rounded-[48px] overflow-hidden"
          style={{ 
            width: MASTER_WIDTH, 
            height: MASTER_HEIGHT, 
      ...
      {/* 4. 하단 메뉴: 짤리지 않도록 위치 최적화 */}
      <div className="absolute bottom-10 left-10 z-[60] flex flex-col gap-4">
        {(stage.inputType === 'direct') && (
          <div className="bg-white/95 backdrop-blur-xl p-5 rounded-3xl shadow-2xl border border-indigo-50 flex flex-col gap-3 min-w-[200px]">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest">입력 감도 조절</label>
            <div className="flex items-center gap-4">
              <input type="range" min="1.0" max="1.8" step="0.1" value={expansionFactor} onChange={(e) => setExpansionFactor(parseFloat(e.target.value))} className="flex-1 h-1.5 bg-slate-200 rounded-lg appearance-none accent-indigo-600 cursor-pointer" />
              <span className="text-[12px] font-black text-indigo-600 w-10 text-right">{Math.round(expansionFactor * 100)}%</span>
            </div>
          </div>
        )}

        {(stage.inputType === 'drag') && (
          <div className="bg-white/95 backdrop-blur-xl p-5 rounded-3xl shadow-2xl border border-indigo-50 flex flex-col gap-4 min-w-[240px]">
            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">텍스트 크기 설정</label>
              <div className="flex gap-4">
                <div className="flex-1 flex flex-col gap-1">
                  <span className="text-[9px] text-slate-400 font-bold uppercase">Pinyin</span>
                  <input type="range" min="20" max="60" value={pinyinSize} onChange={(e) => setPinyinSize(parseInt(e.target.value))} className="h-1 bg-slate-200 rounded-lg appearance-none accent-indigo-600 cursor-pointer" />
                </div>
                <div className="flex-1 flex flex-col gap-1">
                  <span className="text-[9px] text-slate-400 font-bold uppercase">Hanja</span>
                  <input type="range" min="60" max="150" value={hanjaSize} onChange={(e) => setHanjaSize(parseInt(e.target.value))} className="h-1 bg-slate-200 rounded-lg appearance-none accent-indigo-600 cursor-pointer" />
                </div>
              </div>
            </div>
            <div className="flex flex-col gap-1.5 border-t border-slate-100 pt-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">콘텐츠 배율</label>
              <input type="range" min="0.5" max="1.2" step="0.05" value={contentScale} onChange={(e) => setContentScale(parseFloat(e.target.value))} className="w-full h-1 bg-slate-200 rounded-lg appearance-none accent-indigo-600 cursor-pointer" />
            </div>
          </div>
        )}
      </div>

      {/* 5. 우측 하단 액션 버튼: 하단 짤림 방지 위치 조정 */}
      <div className="absolute bottom-10 right-10 z-[60] flex gap-4">
        <button 
          onClick={() => { fCanvas.clear(); fCanvas.backgroundColor='#ffffff'; renderWritingStage(); if(stage.inputType === 'drag') renderDragStage(); }} 
          className="h-14 px-8 bg-white border border-slate-200 rounded-2xl font-black text-slate-600 shadow-xl hover:bg-slate-50 hover:border-slate-300 transition-all active:scale-95"
        >
          초기화
        </button>
        <button 
          onClick={onNext} 
          className="h-14 px-10 bg-indigo-600 text-white rounded-2xl font-black shadow-2xl shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95 flex items-center gap-2"
        >
          <span>입력 완료</span>
